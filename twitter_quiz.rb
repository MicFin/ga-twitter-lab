...
if (user_name.class == String) && (user_name2.class == String)
  ...
  if is_following && is_followed_by
    ...
    if tweet_at == "yes"
      ## code to send tweet
      client.update("@#{user_name2} spread the love and follow me back!")
    end
  else 
    ## is followed by but not following
    ...
    if start_follow == "yes"
      ...
      client.follow(user_name2.to_s)
    end
  end
  if client.user(user_name.to_s).followers_count > client.user(user_name2.to_s).followers_count
    puts "#{user_name} is more popular by #{client.user(user_name.to_s).followers_count - client.user(user_name2.to_s).followers_count} followers"
    
  elsif 
    client.user(user_name.to_s).followers_count < client.user(user_name2.to_s).followers_count
    puts "#{user_name} is more popular by #{client.user(user_name.to_s).followers_count - client.user(user_name2.to_s).followers_count} followers"
  else
    puts "Both peeps have #{client.user(user_name.to_s).followers_count} followers...weird!"
  end
